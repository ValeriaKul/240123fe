...
//! Задача - написать функцию `makeRequest`, которая имитирует загрузку данных товаров с "бэка". Функция должна вернуть промис, который будет зарезолвлен массивом фейковых товаров `fakeProducts` с задержкой в 3 секунды. Результат вывести в консоль.
//! Имитируем получение полноценного ответа от "бэка". Теперь, нам нужно резолвить наш промис из п.1 объектом `successResponse`.
...
makeRequest()
  ...
  .then((data) => {
    const totalPrice_1 = data.filter((product) => product.price < 50)
